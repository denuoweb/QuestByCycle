...
def _prepare_quests(game, user_id, user_quests, now):
    ...
    calendar_quests = [q for q in quests if q.calendar_event_id]
    non_calendar = [q for q in quests if not q.calendar_event_id]
    ...
    calendar_quests.sort(key=lambda x: x.event_start or datetime.max)
