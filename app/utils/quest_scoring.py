"""Functions related to quest scoring and badge awards."""
...
from app.models import db, Quest, Badge, UserQuest, User, ShoutBoardMessage, QuestSubmission, Game

MAX_POINTS_INT = 2**63 - 1


def update_user_score(user_id: int) -> bool:
    """Recalculate and store a user's total points."""
...
def getLastRelevantCompletionTime(user_id: int, quest_id: int):
    now = datetime.now(UTC)
    quest = db.session.get(Quest, quest_id)
...
def check_and_award_badges(user_id: int, quest_id: int, game_id: int) -> None:
    """Award quest and category badges for the given quest."""
    ...
    if quest.badge and user_quest.completions >= quest.badge_awarded:
        existing_award = (
            ShoutBoardMessage.query.filter_by(user_id=user_id, game_id=game_id)
            .filter(ShoutBoardMessage.message.contains(f"data-badge-id='{quest.badge.id}'"))
            .first()
        )
    ...
    if quest.category and game_id:
        ...
        if len(completed_quests) == len(category_quests):
            ...
            for badge in category_badges:
                existing_award = (
                    ShoutBoardMessage.query.filter_by(user_id=user_id, game_id=game_id)
                    .filter(ShoutBoardMessage.message.contains(f"data-badge-id='{badge.id}'"))
                    .first()
                )
                if not existing_award:
...
def check_and_revoke_badges(user_id: int, game_id: int | None = None) -> None:
    """Remove badges when their conditions are no longer met."""
...
def enhance_badges_with_task_info(badges, game_id: int | None = None, user_id: int | None = None):
    """Return badge info enhanced with quest data."""
...
def get_game_badges(game_id: int):
