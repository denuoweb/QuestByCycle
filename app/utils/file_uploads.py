"""Utility helpers for handling uploaded media files."""
...
def _get_ffmpeg_bin() -> str | None:
    """Return the path to a usable ``ffmpeg`` binary if available."""
...
def allowed_file(filename: str, allowed_extensions: Iterable[str]) -> bool:
    """Return ``True`` if ``filename`` has one of ``allowed_extensions``."""

    return "." in filename and filename.rsplit(".", 1)[1].lower() in allowed_extensions


def allowed_image_file(filename: str) -> bool:
    """Return ``True`` if ``filename`` is an allowed image type."""

    return allowed_file(filename, ALLOWED_IMAGE_EXTENSIONS)


def allowed_video_file(filename: str) -> bool:
    """Return ``True`` if ``filename`` is an allowed video type."""

    return allowed_file(filename, ALLOWED_VIDEO_EXTENSIONS)


def correct_image_orientation(img: Image.Image) -> Image.Image:
    """Return a copy of ``img`` with its EXIF orientation applied."""
...
def save_image_file(
    ...
    *,
    ...
    """Save ``image_file`` under ``static/<subpath>`` and return its path."""
...
def save_leaderboard_image(image_file):
    ...
    except Exception as e:  # pragma: no cover - just re-raises
        raise ValueError(f"Failed to save image: {e}") from e


def create_smog_effect(image: Image.Image, smog_level: float) -> Image.Image:
...
def generate_smoggy_images(image_path: str, game_id: int) -> None:
    try:
        ...
        raise ValueError(f"Failed to generate smoggy images: {e}") from e


def save_profile_picture(profile_picture_file, old_filename: str | None = None) -> str:
...
def save_badge_image(image_file) -> str:
    try:
        return os.path.basename(
            save_image_file(image_file, os.path.join("images", "badge_images"), output_ext="png")
        )
    except Exception as e:  # pragma: no cover - just re-raises
        raise ValueError(f"Failed to save image: {e}") from e


def save_bicycle_picture(bicycle_picture_file, old_filename: str | None = None) -> str:
    subdir = os.path.join(current_app.config["main"]["UPLOAD_FOLDER"], "bicycle_pictures")
    return save_image_file(bicycle_picture_file, subdir, old_filename=old_filename)


def save_submission_image(submission_image_file) -> str:
    try:
        return save_image_file(submission_image_file, os.path.join("images", "verifications"))
    except Exception as e:
        current_app.logger.error("Failed to save image: %s", e)
        raise


def save_submission_video(submission_video_file) -> str:
    ...
    try:
        ...
        current_app.logger.error("Failed to save video: %s", e)
        raise
...
def save_sponsor_logo(image_file, old_filename: str | None = None) -> str:
